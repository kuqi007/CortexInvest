...
| 创业板 | **新增**，代码 `399006` | eastmoney 实时行情 |
| 科创50 | **新增**，代码 `000688` | eastmoney 实时行情 |
| 成交额 | 现有 `marketTurnover.total`，单位万亿 | 不变 |
| AMO1 | 现有 `marketTurnover.amo1` | 6日均量比 |
| AMO2 | 现有 `marketTurnover.amo2` | 12日均量比 |
| 趋势箭头 | AMO1 > AMO2 → ↑，否则 → ↓ | |
...
```
恒生: 24000.33 +0.52% | 恒生科技: 5800.10 -0.31% | 成交额: 200亿  AMO1=0.8x AMO2=1.1x ↓
```
...
| AMO1/AMO2 | 现有 `marketTurnover.amo1/amo2`（港股口径） | 港股日成交额数据源口径与A股不同，显示仅作参考 |
...
- 删除内嵌的大盘行（lines 441–456 附近的 A 股 vol 行）
- 删除 FX 汇率行（lines 432–439）
