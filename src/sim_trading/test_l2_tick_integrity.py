...
class TestL2TickIntegrity:
    ...
    def _make_session_json(self, tick_imbalance, buy_vol, sell_vol, 
                           lo_buy=0, lo_sell=0, cf_inflow=0):
        ...
        return json.dumps({
            ...
            "tick": {
                ...
                "direction_score": 1 if tick_imbalance > 0.1 else -1 if tick_imbalance < -0.1 else 0
            },
            "large_order": {
                "buy_count": lo_buy,
                "sell_count": lo_sell,
                ...
                "direction_score": 1 if lo_buy > lo_sell else -1 if lo_sell > lo_buy else 0,
                "orders": []
            },
            "capital_flow": {
    ...
    def test_compute_l2_digest_uses_1600_cutoff(self, mock_db, monkeypatch):
        ...
        session_1555 = self._make_session_json(tick_imbalance=0.25, buy_vol=1000000, sell_vol=600000)
        conn.execute(
        ...
        session_1610 = self._make_session_json(tick_imbalance=0.0, buy_vol=0, sell_vol=0)
        conn.execute(
            "INSERT INTO session_snapshots (ts, date, time, code, session_json) VALUES (?, ?, ?, ?, ?)",
    ...
    def test_compute_l2_digest_excludes_a_shares(self, mock_db, monkeypatch):
        ...
        session = self._make_session_json(tick_imbalance=0.15, buy_vol=500000, sell_vol=400000)
        conn.execute(
            "INSERT INTO session_snapshots (ts, date, time, code, session_json) VALUES (?, ?, ?, ?, ?)",
    ...
    def test_build_llm_prompt_includes_micro_data_whitelist(self, monkeypatch):
        ...
        stats = {"totalSignals": 10, "totalAlerts": 5, "stockCount": 3, "l1Count": 8, "bullish": 3, "bearish": 2, "upCount": 25, "downCount": 15}
        per_stock = [
            {"code": "HK00700", "name": "腾讯", "type": "holding", "change": 1.0,
             "signalCount": 2, "direction": "偏多", "keySignals": ["test"],
             "star": True, "mkt_val": 100000, "cost": 500.0, "shares": 200, "pnl_pct": 5.0},
            {"code": "600673", "name": "东阳光", "type": "holding", "change": -0.5,
             "signalCount": 0, "direction": "中性", "keySignals": [],
             "star": False, "mkt_val": 50000, "cost": 30.0, "shares": 1000, "pnl_pct": -2.0}
        ]
        l1_displays = []
    ...
    def test_digest_direction_score_calculation(self, mock_db, monkeypatch):
        ...
        session = self._make_session_json(
            tick_imbalance=0.30, buy_vol=1000000, sell_vol=400000,
            lo_buy=5, lo_sell=1, cf_inflow=10000000
        )
        conn.execute(
            "INSERT INTO session_snapshots (ts, date, time, code, session_json) VALUES (?, ?, ?, ?, ?)",
            (ts, date_str, "15:30:00", "HK00700", session)
...
class TestTickDataEdgeCases:
    ...
    def test_zero_tick_volume_handling(self, mock_db, monkeypatch):
        ...
        session = json.dumps({
            "direction": "neutral",
            "score": 0,
            "tick": {"buy_vol": 0, "sell_vol": 0, "imbalance": 0.0, "direction_score": 0},
            "large_order": {"buy_count": 0, "sell_count": 0, "buy_amount": 0, "sell_amount": 0,
                           "net_amount": 0, "direction_score": 0, "orders": []},
            "capital_flow": {"main_net_inflow": 1000000, "main_net_inflow_pct": 2.0, "direction_score": 1}
        })
        conn.execute(
            "INSERT INTO session_snapshots (ts, date, time, code, session_json) VALUES (?, ?, ?, ?, ?)",
            (ts, date_str, "15:30:00", "HK00700", session)
