...
| `market_data.json` | 实时行情 | ✅ 用于计算浮盈浮亏 |
...
2. **读取持仓状态**
   从 `market_data.json` 读取当前行情，与 `config.db:monitor_watchlist` 持仓（list_type='holding'）对比计算盈亏
...
4. **生成复盘报告**
   ...
   ### 持仓盈亏
   逐只列出持仓成本/现价/盈亏%（来源：config.db + market_data.json）
