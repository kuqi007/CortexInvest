...
1. 读取上下文（并行）
   ...
   - trading.db:price_snapshots: 最新行情
   - .claude/notes/{CODE}.md: 个股历史分析（如果存在）
   - .claude/daily/前一天.md: 昨日操作计划（如果存在）
   - trading.db:trade_plans: 已有计划
   ⚠️ 不读 trading.db trades/live_state（纯模拟，复盘不看）
    ↓
...
4. 输出格式
   ...
   | 仓位比例 | 建议占总仓位 x% |
    ↓
5. 更新持久化
   - 个股分析 → 追加到 .claude/notes/{CODE}.md（倒序，新日期最上方）
   - 每日操作计划 → 写入 .claude/daily/YYYY-MM-DD.md（当日复盘+次日操作）
   - 如需自动触发 → `POST /api/trade-plans` / `trading.db:trade_plans`
   - 如需提醒 → `POST /api/config` / `config.db:alert_rules`
```
